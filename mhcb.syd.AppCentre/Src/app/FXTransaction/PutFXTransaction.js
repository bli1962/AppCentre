$(document).ready(function () {
    ...
    if (!auth.includes("FX-Trans - Authorise")) {
        $('#btnUpdate').prop('disabled', true);
        alert("Sorry, you don't have FX Transaction authorisd permission");
    }
    ...
    $('#btnRefresh').click(function () {
        ...
        $.ajax({
            type: 'GET',
            url: origin + '//api/FXTransaction/getPendingFxTrans',
            //url: dev + '/api/FXTransaction/Pending',
            //url: deploy + '/api/FXTransaction/Pending',
            dataType: 'json',
            data: {},
            //*****************************************************************************
            ...
            success: function (result) {
                ...
                $.each(result, function (index, value) {
                    var row = $('<tr><td>'
                        ...
                        + value.DELIVARY_DATE + '</td><td>'
                        + value.SETTLEMENT_VALUE_DATE + '</td><td>'
                        + value.CONTRACT_CCY_ABBR + '</td><td>'
                        + value.CONTRACT_CCY_AMT + '</td><td>'
                        + value.EQUIV_CCY_ABBR + '</td><td>'
                        + value.STATUS + '</td><td>'
        ...
        $.ajax({
            ...
            error: function (jQXHR) {
                ...
                if (jQXHR.status == "401") {
                    ...
                    $('#divMessageText').text(jQXHR.responseText);
                    $('#divModalHeader').text("FX Transaction");
                    $('#divModalBody').text("It's not your FX Transaction!");
                }
                else {
                    $('#divMessageText').text(jQXHR.responseText);
                    $('#divModalHeader').text("FX Transaction");

$(document).ready(function () {
    ...
    $('#btnRefresh').click(function () {
        ...
        $.ajax({
            type: 'GET',
            url: origin + '//api/Payment/getPPTransByDate',
            //url: dev + '/api/Payment',
            //url: deploy + '/api/Payment',
            dataType: 'json',
            data: {
            ...
            success: function (result) {
                ...
                $.each(result, function (index, value) {
                    var row = $('<tr><td>'
                        ...
                        + value.PaymentCurrencyCode + '</td><td>'
                        + value.Amount + '</td></tr>'
            ...
            error: function (jQXHR) {
                ...
                if (jQXHR.status == "401") {
                    ...
                    $('#divMessageText').text(jQXHR.responseText);
                    $('#divModalHeader').text("PaymentControl");
    ...
    $('#btnUpdate').click(function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $('#divMessageText').text("Change Payment Control Status Successfully");
               // $('#divMessageBox').show('fade');

                // and
                $('#divModalFooter').addClass("btn-primary");
                $('#divModalHeader').text("PaymentControl");
                $('#divModalBody').text("Change Payment Control Status Successfully");
                $('#successModal').modal('show');
            },

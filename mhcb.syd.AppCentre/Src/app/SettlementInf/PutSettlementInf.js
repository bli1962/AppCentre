$(document).ready(function () {
    ...
    if (!auth.includes("SIF - Authorise")) {
        $('#btnUpdate').prop('disabled', true);
        alert("Sorry, you don't have SIF authorisd permission");
    }
    ...
    $('#btnRefresh').click(function () {
        ...
        $.ajax({
            type: 'GET',
            url: origin + '//api/SettlementInf/getPendingSIF',
            //url: dev + '/api/SettlementInf/Pending',
            //url: deploy + '/api/SettlementInf/Pending',
            dataType: 'json',
            data: {},
            //*****************************************************************************
    ...
    $('#btnUpdate').click(function () {
        ...
        if ($('#sifId').val() == '') {
            $('#divMessageBox').addClass('alert-warning');
            $('#divMessageText').text("Please enter SID ID");
           // $('#divMessageBox').show('fade');
            return false;
        }
        if ($('#tranNo').val() == '') {
        ...
        $.ajax({
            type: 'PUT',
        /*    url: origin + '//api/settlementinf?id=3240',*/
            url: origin + '//api/SettlementInf/',
            dataType: 'json',
            data: {
                SIF_ID: $('#sifId').val(),
            ...
            error: function (jQXHR) {
                ...
                if (jQXHR.status == "401") {
                    ...
                    $('#divMessageText').text(jQXHR.responseText);
                    $('#divModalHeader').text("Settlement Information");
                    $('#divModalBody').text("It's not your Settlement Information!");
